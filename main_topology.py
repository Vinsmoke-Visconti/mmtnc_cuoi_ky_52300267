...
def run():
    ...
    info('\n--- Can bang tai ---\n')
    info('  py exec(open("scripts/load_balancer.py").read() + "\\ndemo_load_balance(net)")\n')
    info('\n--- Do hieu nang ---\n')
    info('  py exec(open("scripts/performance_test.py").read() + "\\nfull_test(net)")\n')
    info('\n--- Kiem tra NAT/ACL ---\n')
    info('  py exec(open("scripts/nat_acl_test.py").read() + "\\nrun_nat_acl_test(net)")\n')
    info('\nGo "exit" de thoat.\n\n')
